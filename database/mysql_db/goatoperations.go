...
func (m *MySqlDB) GetGOATStats(season string) (*sql.Rows, error) {
	return m.db.Query("SELECT p.playerid, p.name, sr.position, sr.pointspergame, sr.reboundspergame, sr.assistspergame, sr.stealspergame, sr.blockspergame, p.allstar, p.allnba, p.alldefense, p.championships, p.dpoy, p.finalsmvp, p.mvp, sr.per, sr.ows, sr.dws, sr.ws, sr.dbpm, sr.obpm, sr.bpm, sr.defrtg, sr.offrtg, sp.pointspergame, sp.reboundspergame, sp.assistspergame FROM goat_players p INNER JOIN goat_stats sr ON p.playerid=sr.playerid INNER JOIN goat_stats sp ON p.playerid=sp.playerid WHERE sp.isplayoffs=1 ORDER BY per DESC")
}
